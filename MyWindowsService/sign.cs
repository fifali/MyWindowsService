...
namespace MyWindowsService
{
    public class sign
    {
        ...
        private static string HandleRequest(HttpListenerRequest request, HttpListenerResponse response)
        {
            ...
            string _data = null;
            string data = null;
            string _SignType = null;
            string halg = null;
            string _mchPrivateKey = null;
            string _prv = null;
            ...
            try
            {
                ...
                if (jObject.Property("SignText") == null)
                {
                    #region 找不到SignText标签返回
                    _rettext = "找不到SignText标签";
                    response.StatusDescription = "200";
                    response.StatusCode = 200;
                    return _rettext;
                    #endregion
                }
                _data = jObject["SignText"].ToString();

                if (jObject.Property("SignType") == null)
                {
                    #region 找不到SignType标签返回
                    _rettext = "找不到SignType标签";
                    response.StatusDescription = "200";
                    response.StatusCode = 200;
                    return _rettext;
                    #endregion
                }
                _SignType = jObject["SignType"].ToString();

                if (jObject.Property("mchPrivateKey") == null)
                {
                    #region 找不到mchPrivateKey标签返回
                    _rettext = "找不到mchPrivateKey标签";
                    response.StatusDescription = "200";
                    response.StatusCode = 200;
                    return _rettext;
                    #endregion
                }
                _mchPrivateKey = jObject["mchPrivateKey"].ToString();

                if (_SignType == "RSA_1_1")
                    halg = "SHA1";
                if (_SignType == "RSA_1_256")
                    halg = "SHA256";
                _prv = RSAConverter.RSAPrivateKeyJava2DotNet(_mchPrivateKey.ToString());
                _sign = RSAHelper.SignData(_prv, _data.ToString(), halg);
            }
            catch (Exception ex)
            {
                response.StatusDescription = "200";
